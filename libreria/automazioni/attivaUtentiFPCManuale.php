...
if(isset($_POST['codice_utente']) && !empty($_POST['codice_utente'])){
    
    unset($_SESSION['processati_import']);
    unset($_SESSION['processati_import_step_2']);
    unset($_SESSION['fpc_id_import']);
    unset($_SESSION['fpc_id_professionista']);
    ...
    if(strlen($richiesta_codice_fiscale)>10){
        ...
        if($row_00002['id']>0 && $row_00002['stato']=="Da Attivare"){
            $insertAttivazione = array(
                "email" => $row_00002['email'],
                "nome" => $row_00002['nome'],
                "cognome" => $row_00002['cognome'],
                "commerciale" => $row_00002['admin'],
                "sezionale" => "00",
                "tipo_marketing" => "SITO FPC",
                "classe" => "Ingegneri",
                "id_classe" => "10",
                ...
                "data_scadenza" => date("Y-m-d", strtotime('+1 years')),
                "stato" => "Importato"
            );
            
            $dblink->insert("lista_attivazioni_manuale", $insertAttivazione);
            $idInsert = $dblink->lastid();
            $idProff = $row_00002['id'];
        
            						
         header('Location:http://attivazione.fpcingegneri.com/attivazione.php?fpc_id='.$idProff.'&fpcImp='.$idInsert.'&step=1');
            die();
        }else{
        	header('Location:http://attivazione.fpcingegneri.com/attivazione.php?step=0&error=2');
    		die();
        }
    }else{
        $idProff = "";
    ...
    header('Location:http://attivazione.fpcingegneri.com/attivazione.php?step=0&error=1');
    die();
}else if(isset($_POST['codice_utente'])){
	header('Location:http://attivazione.fpcingegneri.com/attivazione.php?step=0&error=1');
    die();
}
...
switch ($step) {
    ...
    default:
        ...
        AND stato='Importato' AND id = '$fpcImp' GROUP BY email, codice_prodotto";
        ...
        foreach ($rs_lista_attivazioni_manuale AS $row_lista_attivazioni_manuale){
            ...
            if($idProfessionista<=0){
                ...
                SET 
                stato = 'ERRORE: ID PROFESSIONISTA NON TROVATO'
                WHERE id = '".$id_lista_attivazioni_manuale."'
                AND stato='Importato'";
                $ok = $dblink->query($sql_aggiorna_lista_attivazioni_manuale);
                
                header('Location:http://attivazione.fpcingegneri.com/attivazione.php?fpc_id='.$fpc_id.'&fpcImp='.$fpcImp.'&step=1&error=1');
                die();
            }else{
                $updateProf = array(
            ...
            if($idProfessionista>0){
                ...
                if($controlloAbbonamento>0){
                    if($id_classe == $rowProfessionista['id_classe']){
                        ...
                        $sql_aggiorna_lista_attivazioni_manuale = "UPDATE lista_attivazioni_manuale 
                        SET 
                        stato = 'ERRORE: ABBONAMENTO NON TROVATO'
                        WHERE id = '".$id_lista_attivazioni_manuale."'
                        AND stato='Importato'";
                        $ok = $dblink->query($sql_aggiorna_lista_attivazioni_manuale);
                        
                        header('Location:http://attivazione.fpcingegneri.com/attivazione.php?fpc_id='.$fpc_id.'&fpcImp='.$fpcImp.'&step=1&error=2');
                        die();
                    }
                }
            }
            ...
            if($idProfessionista>0){
                ...
                $insert = array(
                    ...
                    "messaggio" => "ATTIVAZIONE MANUALE DI FPC - ID ($id_lista_attivazioni_manuale)",
        ...
        //echo "http://attivazione.fpcingegneri.com/attivazione.php?fpc_id='.$fpc_id.'&fpcImp='.$fpcImp.'&step=2";
        echo '<meta http-equiv="refresh" content="0; url=http://attivazione.fpcingegneri.com/attivazione.php?fpc_id='.$fpc_id.'&fpcImp='.$fpcImp.'&step=2" ><!--<h3>Processati: '.$processati.'</h3>-->';
    ...
    case 1:
        ...
        UNIX_TIMESTAMP(data_scadenza) AS data_scadenza_corso_timestamp
        FROM lista_attivazioni_manuale WHERE id_utente_moodle <=0 
        AND stato='In Attesa di Venduto' AND id = '$fpcImp' GROUP BY email, codice_prodotto LIMIT 1";
        //$sql_lista_attivazioni_manuale = "SELECT * FROM lista_attivazioni_manuale WHERE id_utente_moodle >0 LIMIT 2";
        $rs_lista_attivazioni_manuale = $dblink->get_results($sql_lista_attivazioni_manuale);
        if(!empty($rs_lista_attivazioni_manuale)){
            ...
            $_SESSION['NOSTRO_HTTP_REFERER'] = "http://attivazione.fpcingegneri.com/attivazione.php?fpc_id=".$fpc_id."&fpcImp=".$fpcImp."&step=3";
            $_SESSION['processati_import'] = $count;
            echo ' <meta http-equiv="refresh" content="0; url='.BASE_URL."/moduli/anagrafiche/salva.php?idPreventivo=$idPreventivo&idCalendario=$idCalendario&fn=preventivoVenduto".'" >';
        ...
        //
            ...
            //echo "HO FINITO";
            echo '<meta http-equiv="refresh" content="0; url=http://attivazione.fpcingegneri.com/attivazione.php?fpc_id='.$fpc_id.'&fpcImp='.$fpcImp.'&step=3" ><!--<h3>Processati: '.$_SESSION['processati_import'].'</h3>-->';
    ...
    case 2:
        ...
        UNIX_TIMESTAMP(data_scadenza) AS data_scadenza_corso_timestamp
        FROM lista_attivazioni_manuale WHERE id_utente_moodle <=0 
        AND stato='In Attesa di Chiuso' AND id='$fpcImp' GROUP BY email, codice_prodotto LIMIT 1";
        //$sql_lista_attivazioni_manuale = "SELECT * FROM lista_attivazioni_manuale WHERE id_utente_moodle >0 LIMIT 2";
        $rs_lista_attivazioni_manuale = $dblink->get_results($sql_lista_attivazioni_manuale);
        if(!empty($rs_lista_attivazioni_manuale)){
            foreach ($rs_lista_attivazioni_manuale AS $row_lista_attivazioni_manuale){
            ...
            $count++;
            //echo "<li>$count - Processo: $email</li><br>";
            $_SESSION['NOSTRO_HTTP_REFERER'] = "http://attivazione.fpcingegneri.com/attivazione.php?fpc_id=".$fpc_id."&fpcImp=".$fpcImp."&step=4";
            $_SESSION['processati_import_step_2'] = $count;
            echo ' <meta http-equiv="refresh" content="0; url='.BASE_URL."/moduli/preventivi/salva.php?tbl=lista_preventivi&idPreventivoFirmato=$idPreventivo&codSezionale=$sezionale&fn=preventivoFirmato".'" >';
            ...
            //echo "HO FINITO";
            echo ' <meta http-equiv="refresh" content="0; url=http://attivazione.fpcingegneri.com/attivazione.php?fpc_id='.$fpc_id.'&fpcImp='.$fpcImp.'&step=4" ><!--<h3>Processati: '.$_SESSION['processati_import'].'</h3>-->';
            //header("Location:".BASE_URL."/libreria/automazioni/autoAttivaUtentiMoodleManuale.php?step=2");
        }
    break;
    ...
    case 3:
        ...
        UNIX_TIMESTAMP(data_scadenza) AS data_scadenza_corso_timestamp
        FROM lista_attivazioni_manuale WHERE id_utente_moodle <=0 
        AND stato='In Attesa di Moodle' AND id='$fpcImp' GROUP BY email, codice_prodotto";
        //$sql_lista_attivazioni_manuale = "SELECT * FROM lista_attivazioni_manuale WHERE id_utente_moodle >0 LIMIT 2";
        $rs_lista_attivazioni_manuale = $dblink->get_results($sql_lista_attivazioni_manuale);
        if(!empty($rs_lista_attivazioni_manuale)){
            foreach ($rs_lista_attivazioni_manuale AS $row_lista_attivazioni_manuale){
                ...
                if(!empty($row_cerca_in_lista_password)){
                    ...
                    SELECT DISTINCT '', NOW(), 'autoAttivaUtentiMoodleManuale', '".$rowProfessionista['id_professionista']."', `id_classe`, 'cliente', `nome`, `cognome`, '".strtolower($rowCodice['codice'])."', '".$password."', `email`, 'In Attesa di Moodle', data_creazione, data_scadenza FROM lista_attivazioni_manuale WHERE id=".$id_lista_attivazioni_manuale." LIMIT 1";
                ...
                if($idUtenteMoodle>0){
                    ...
                    $sql_aggiorna_lista_professionisti = "UPDATE lista_professionisti
                    SET id_moodle_user = '".$idUtenteMoodle."' , 
                    stato = 'Attivo',
                    dataagg = NOW()
                    WHERE id = '".$rowProfessionista['id_professionista']."'";
                    $ok = $dblink->query($sql_aggiorna_lista_professionisti);
        ...
        echo ' <meta http-equiv="refresh" content="0; url=http://attivazione.fpcingegneri.com/attivazione.php?fpc_id='.$fpc_id.'&fpcImp='.$fpcImp.'&step=5&mail='.base64_encode($email).'" >';
