...
if(isset($_GET['filtroAttivi']) && $_GET['filtroAttivi'] == "1"){
    $whereFiltroAttivi = " AND stato LIKE 'Attivo' ";
}else if(isset($_GET['filtroAttivi']) && $_GET['filtroAttivi'] == "0"){
    $whereFiltroAttivi = " AND stato LIKE 'Da Attivare' ";
}else{
	$whereFiltroAttivi = "";
}
...
switch($tabella){
        ...
        case 'lista_professionisti':
            ...
            $campi_visualizzati ="CONCAT('<a class=\"btn btn-circle btn-icon-only yellow btn-outline\" href=\"dettaglio.php?tbl=lista_professionisti&id=',id,'\" title=\"DETTAGLIO\" alt=\"DETTAGLIO\"><i class=\"fa fa-search\"></i></a>') AS 'fa-search',
                            ...
                            CONCAT(IF(cellulare>0, CONCAT('Cel: ',cellulare),''),IF(cellulare > 0 AND telefono > 0, '<br>', ''),IF(telefono>0, CONCAT('Tel: ',telefono),''),'Email: ',email) AS Contatti, stato";
            $where = $table_listaProfessionisti['index']['where'].$whereFiltroAttivi;
            if(!empty($arrayCampoRicerca)){
                foreach ($arrayCampoRicerca as $campoRicerca) {
                    $campoRicerca = $dblink->filter($campoRicerca);
                    ...
                    $where.= " OR telefono LIKE '%".$campoRicerca."%' OR email LIKE '%".$campoRicerca."%' OR stato LIKE '%".$campoRicerca."%')";
                }
            }
